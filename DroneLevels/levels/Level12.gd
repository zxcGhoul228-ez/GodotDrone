...
func _init():
	level_number = 12
	level_title = "УРОВЕНЬ"
	level_hint = "Экспертная доставка: захват, вертикальный обход и точный сброс."
	completion_text = "Контейнер доставлен по экспертному маршруту!"
	start_grid = Vector2i(-4, -4)
	start_height = 8
	target_grid = Vector2i(4, 4)
	target_height = 112
	mission_time_limit_sec = 82
	requires_cargo_delivery = true
	cargo_start_grid = Vector2i(-2, -1)
	cargo_start_height = 32
	cargo_drop_grid = Vector2i(4, 4)
	cargo_drop_height = 96
