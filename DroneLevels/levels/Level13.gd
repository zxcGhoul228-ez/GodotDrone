...
func _init():
	level_number = 13
	level_title = "УРОВЕНЬ"
	level_hint = "Очень сложно: забери объект в центре и вынеси на верхнюю платформу."
	completion_text = "Невероятно! Ты выполнил сложную транспортировку."
	start_grid = Vector2i(-5, -4)
	start_height = 8
	target_grid = Vector2i(5, 4)
	target_height = 120
	mission_time_limit_sec = 76
	requires_cargo_delivery = true
	cargo_start_grid = Vector2i(-1, 0)
	cargo_start_height = 40
	cargo_drop_grid = Vector2i(5, 4)
	cargo_drop_height = 104
