...
@onready var btn_save: Button = $TopBar/SaveButton
@onready var btn_auto: Button = $TopBar/AutoButton
...
func _build_graph() -> void:
	...
	for i in range(motor_count):
		...
		# вход от платы (slot 0: input)
		mn.set_slot(0, true, 0, Color.WHITE, false, 0, Color.WHITE)
		# выход к пропеллеру (slot 1: output)
		mn.set_slot(1, false, 0, Color.WHITE, true, 0, Color.WHITE)
		graph.add_child(mn)
...
func _on_connection_request(from_node: StringName, from_port: int, to_node: StringName, to_port: int) -> void:
	# разрешаем только Board(0 out) -> Motor(0 in) и Motor(1 out) -> Prop(0 in)
...
func _on_auto_pressed() -> void:
	# Автосборка: Board->all motors, motors->same index prop
	var motor_count: int = int(drone_info.get("motor_count", 0))
	var prop_count: int = int(drone_info.get("propeller_count", 0))
...
func _update_status() -> void:
	# простая проверка: все моторы должны иметь вход от Board, и если есть проп — быть подключён
	var motor_count: int = int(drone_info.get("motor_count", 0))
	var prop_count: int = int(drone_info.get("propeller_count", 0))
...
func _on_back_pressed() -> void:
	# верни на свою сцену создания (путь поменяй если другой)
