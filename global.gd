...
# Награды за звезды
var star_rewards = {
	1: 25,   # 1 звезда - 25 очков
	2: 50,   # 2 звезды - 50 очков
	3: 100   # 3 звезды - 100 очков
}
...
# ==================== ОБНОВЛЕННАЯ ФУНКЦИЯ РАСЧЕТА НАГРАДЫ ====================
func calculate_level_reward(level: int, actual_time_ms: int, first_time: bool) -> Dictionary:
	...
	# Добавляем бонус за первое прохождение, если есть хотя бы 1 звезда
	...
	return {
		...
		"bonus": first_time_bonus if (first_time and stars > 0) else 0
	}
# ==================== ОБНОВЛЕННАЯ ФУНКЦИЯ ЗАВЕРШЕНИЯ УРОВНЯ ====================
func complete_level(level_number: int, time_ms: int):
	...
	if level_key in levels_data:
		...
		# Рассчитываем награду для этого прохождения
		...
		# Начисляем очки только при первом прохождении или улучшении результата
		...
		if not was_completed or is_improvement:
			add_score(reward)  # Используем add_score вместо add_currency
...
# Функция для добавления очков (замените add_currency на add_score)
func add_score(amount: int):
	score += amount
	save_game()
	print("💰 Получено очков: ", amount, ". Всего: ", score)

# Функция получения очков (если нужна)
func get_score() -> int:
	return score
...
func save_game():
	...
	# УДАЛИТЬ: config.set_value("game", "currency", currency)
...
func load_game():
	...
	if error == OK:
		...
		purchased_items = config.get_value("game", "purchased_items", ["Рама1", "Плата1", "Мотор1", "Пропеллер1"])
