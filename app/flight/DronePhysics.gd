...
func get_visual_tilt_degrees(direction: int = -1) -> Vector3:
	...
	var drift_strength: float = clampf((1.0 - stability) * 0.85 * get_imbalance_multiplier(), 0.0, 1.0)
...
func apply_movement_physics(direction: int, current_pos: Vector3, target_pos: Vector3, delta: float, speed: float = 32.0) -> Vector3:
	...
	if drift_dir != Vector3.ZERO:
		...
		var drift_fraction: float = clampf((1.0 - stability) * 0.28 * imbalance_mul, 0.0, 0.62)
	...
	if missing_count > 0:
		...
		var sink_strength: float = clampf(missing_frac * (0.28 + 0.72 * (1.0 - stability)), 0.0, 1.0)
		var sink_speed: float = float(GRAVITY) * 0.20 * sink_strength
		next_pos.y -= sink_speed * safe_delta
		velocity.y -= sink_speed * 0.35 * safe_delta

	if stability < 0.35:
		next_pos.y -= (0.35 - stability) * 0.30 * float(GRAVITY) * safe_delta
