...
var motors: Array = []  # {position: Vector3, thrust: float, has_propeller: bool, motor_index: int}
...
const MOTOR_POSITIONS = [
	Vector3(0.5, 0, 0.5),   # Передний правый
	Vector3(-0.5, 0, 0.5),  # Передний левый
	Vector3(0.5, 0, -0.5),  # Задний правый
	Vector3(-0.5, 0, -0.5)  # Задний левый
]
...
func setup_from_components(frame, board, motor_nodes: Array, propeller_nodes: Array):
	...
	if frame and is_instance_valid(frame):
		# Получаем тип рамы из метаданных или имени
		...
		positions.append(Vector3.ZERO)  # Рама в центре
		print("📊 Рама: ", frame_type, " масса: ", frame_mass)
	
	# Плата
	if board and is_instance_valid(board):
		# Получаем тип платы из метаданных или имени
		...
		positions.append(Vector3(0, 0.1, 0))  # Немного выше
	...
	for i in range(motor_nodes.size()):
		...
		if motor and is_instance_valid(motor):
			...
			var motor_data = {
				"position": MOTOR_POSITIONS[i] if i < MOTOR_POSITIONS.size() else Vector3.ZERO,
				"thrust": motor_thrust,
				"has_propeller": false,
				"index": i,
			...
			print("📊 Мотор ", i, ": ", motor_type, " тяга: ", motor_thrust, " масса: ", motor_mass)
	...
	for i in range(propeller_nodes.size()):
		...
		if propeller and is_instance_valid(propeller):
			...
			# Находим ближайший мотор для этого пропеллера
			...
			var propeller_data = {
				"position": MOTOR_POSITIONS[closest_motor_index] if closest_motor_index < MOTOR_POSITIONS.size() else Vector3.ZERO,
			...
			# Помечаем мотор как имеющий пропеллер
			if closest_motor_index < motors.size():
				motors[closest_motor_index]["has_propeller"] = true
				# Сохраняем эффективность пропеллера в данных мотора
			...
			print("📊 Пропеллер ", i, ": ", propeller_type, " эффективность: ", propeller_efficiency, " масса: ", propeller_mass)
	...
	print("  Моторов с пропеллерами: ", get_active_motors_count())
...
func calculate_lift_and_balance():
	...
	# Рассчитываем тягу от каждого мотора с пропеллером
	for motor in motors:
		if motor["has_propeller"]:
			# Эффективная тяга = базовая тяга мотора * эффективность пропеллера
	...
	if total_thrust > 0:
		...
		# Вектор разбалансировки - от центра масс к центру тяги
	...
	# Если пропеллеров меньше чем моторов, уменьшаем подъемную силу
	if propellers.size() < motors.size():
		lift_capacity *= float(propellers.size()) / float(motors.size())

func calculate_thrust_vector(throttle: float = 1.0) -> Vector3:
	"""Рассчитывает вектор тяги с учетом разбалансировки"""
	var base_thrust = Vector3.UP * lift_capacity * throttle
	
	if not is_stable and thrust_imbalance.length() > 0:
		# Добавляем смещение из-за разбалансировки
		# Чем больше разница, тем сильнее крен
		var imbalance_strength = thrust_imbalance.length()
		var tilt_vector = thrust_imbalance.normalized() * imbalance_strength * 0.5
		
		# Вращаем вектор тяги в сторону разбалансировки
		var tilted_thrust = base_thrust.rotated(Vector3.RIGHT, tilt_vector.x * 0.3)
		tilted_thrust = tilted_thrust.rotated(Vector3.FORWARD, tilt_vector.z * 0.3)
		
		return tilted_thrust
	else:
		return base_thrust

func can_take_off() -> bool:
	"""Проверяет, может ли дрон взлететь"""
	# Минимальные условия для взлета:
	# 1. Хотя бы 2 мотора с пропеллерами для базовой стабильности
	# 2. Подъемная сила > массы * 1.1 (10% запас)
	var active_motors = get_active_motors_count()
	
	return active_motors >= 2 and lift_capacity > total_mass * 1.1
...
func get_flight_behavior() -> Dictionary:
	...
	# Рассчитываем стабильность
	if motors.size() == 0:
		behavior["stability"] = 0.0
	elif active_motors < 2:
		behavior["stability"] = 0.3
	elif active_motors == 2:
		behavior["stability"] = 0.6
	elif active_motors == 3:
		behavior["stability"] = 0.8
	else:
		behavior["stability"] = 1.0
...
func apply_physics(delta: float, current_position: Vector3, target_position: Vector3) -> Vector3:
	"""Применяет физику к движению дрона"""
	if not is_airborne:
		return target_position
	
	var behavior = get_flight_behavior()
	
	if not behavior["can_fly"]:
		# Дрон не может лететь - падает вниз
		return current_position + Vector3(0, -GRAVITY * delta * 5, 0)
	
	# Рассчитываем базовое движение
	var direction = (target_position - current_position).normalized()
	var base_movement = direction * behavior["max_speed"] * delta * 10
	
	# Добавляем дрейф из-за разбалансировки
	if behavior["drift_direction"].length() > 0:
		var drift_strength = (1.0 - behavior["stability"]) * 2.0
		var drift = behavior["drift_direction"] * drift_strength * delta * 5
		base_movement += drift
	
	# Добавляем случайные колебания при низкой стабильности
	if behavior["stability"] < 0.7:
		var turbulence = Vector3(
			randf_range(-1, 1) * (1.0 - behavior["stability"]),
			randf_range(-0.5, 0.5) * (1.0 - behavior["stability"]),
			randf_range(-1, 1) * (1.0 - behavior["stability"])
		) * delta * 3
		base_movement += turbulence
	
	# Рассчитываем конечную позицию
	var new_position = current_position + base_movement
	
	# Проверяем, не падает ли дрон
	if behavior["stability"] < 0.4:
		# Дрон теряет высоту
		new_position.y -= (0.5 - behavior["stability"]) * GRAVITY * delta * 2
...
func check_crash_condition(current_position: Vector3, ground_level: float = 0.0) -> bool:
	...
	if current_position.y < ground_level - 5.0:
		return true
	
	# 2. Дрон слишком нестабилен и падает
	if behavior["stability"] < 0.2:
		return true
